...
def find_label_area_from_generated_mask(output_mask, image_shape, image_rgb, save_path="result/processed_image.png"):
    ...
    Returns:
        ...
        Blackens the detected label region on the original image.sc
    """
    ...
    for mask_data in output_mask:
        ...
        # Select the closest bounding box to an edge that also meets the area constraint
    ...
    if best_box:
        ...
        image_with_box[y:y+h, x:x+w] = (0, 0, 0)  # Setting the area to black

        print(f"Detected Label Region - x:{x}, y:{y}, w:{w}, h:{h}, Area: {area}")
        ...
        #cv2.rectangle(image_with_box, (x, y), (x + w, y + h), (255, 0, 0), 2)  # Blue rectangle
        plt.imshow(image_with_box)
        plt.axis("off")
        plt.show()
        return area
    else:
        print("No suitable label region found (either too far or too large).")
        return None
